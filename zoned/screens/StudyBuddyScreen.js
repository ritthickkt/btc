    ...
    <View style={styles.card}>
      ...
      {!isOwn && (
        <View style={styles.cardActions}>
          ...
          >
            <Ionicons name={alreadySent ? 'checkmark' : 'person-add'} size={14} color="#1a1a1a" />
      ...
      {isOwn && (
        <View style={styles.ownPostLabel}>
    ...
    <View style={styles.card}>
      ...
      <View style={styles.cardActions}>
        <TouchableOpacity style={styles.acceptBtn} onPress={() => onAccept(request)} activeOpacity={0.8}>
          <Ionicons name="checkmark-circle" size={15} color="#1a1a1a" />
...
export default function StudyBuddyScreen({ navigation }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [postsRes, requestsRes, sentRes] = await Promise.all([
      ...
      supabase.from('study_requests')
        .select('*, study_posts(course, location, topic)')
  ...
  // Runs on initial focus and when navigating back from a non-modal screen
  useFocusEffect(
    useCallback(() => {
      fetchData();
    }, [fetchData])
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('PostStudy')}
          activeOpacity={0.85}
        >
      ...
      <View style={styles.toggleWrap}>
        <TouchableOpacity
          style={[styles.toggleBtn, activeTab === 'browse' && styles.toggleBtnActive]}
          onPress={() => setActiveTab('browse')}
          activeOpacity={0.8}
        >
          <Ionicons name="search-outline" size={15} color={activeTab === 'browse' ? '#1a1a1a' : '#999'} />
          <Text style={[styles.toggleLabel, activeTab === 'browse' && styles.toggleLabelActive]}>Browse</Text>
        </TouchableOpacity>
        <TouchableOpacity
          ...
          activeOpacity={0.8}
        >
          <Ionicons name="mail-outline" size={15} color={activeTab === 'requests' ? '#1a1a1a' : '#999'} />
          <Text style={[styles.toggleLabel, activeTab === 'requests' && styles.toggleLabelActive]}>Requests</Text>
          {requests.length > 0 && (
            <View style={styles.badgeDot}>
              <Text style={styles.badgeDotText}>{requests.length}</Text>
            </View>
          )}
        </TouchableOpacity>
      </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.empty}>
              ...
              <Text style={styles.emptySubText}>When someone sends you a request, it'll appear here</Text>
            </View>
          }
        />
      )}

      {/* Send Request message sheet */}
      {requestTarget && (
        <>
          <TouchableOpacity style={styles.sheetBackdrop} onPress={closeRequestSheet} activeOpacity={1} />
            ...
            <Animated.View style={[styles.sheet, { transform: [{ translateY: sheetAnim }] }]}>
              ...
              >
                {sending
                  ...
                  : <Text style={styles.sheetSendBtnText}>Send Request</Text>
                }
              </TouchableOpacity>
            </Animated.View>
          </KeyboardAvoidingView>
